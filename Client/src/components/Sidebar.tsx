        ...
        <div className='flex flex-col h-full'>
                    ...
                    if(isMessage){
                        ...
                        return (
                            <div key={msg.id} className={`flex items-center gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
                                    ...
                                    <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-s leading-relaxed ${isUser ? 'bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none ' : ' rounded-tl-none bg-gray-800 text-gray-100'}`}>
                                        {msg.content}
                                    </div>
                                    {isUser && (
                            ...
                            <div key={ver.id } className='w-4/5 mx-auto my-2 p-3 rounded-xl bg-gray-800 text-gray-100 shadow flex flex-col gap-2'>
                                    ...
                                    <div className='flex items-center justify-between'>
                                        ...
                                        <Link target='_blank' to={`/preview/${project.id}/${ver.id}`}>
                                            <EyeIcon className='size-6 py-1 bg-gray-700 hover:bg-indigo-700 transition-colors rounded' />
                                        </Link>
                                    </div>
                            </div>
                        )
            ...
            <form onSubmit={handleRevisions} className='m-3 relative'>
                <div className='flex items-center gap-2'>
                    <textarea onChange={(e)=>setInput(e.target.value)} value={input} rows={4} placeholder='Describe your website or request changes...' className='flex-1 p-3 rounded-xl resize-1 text-sm outline-none ring ring-gray-700 focus:ring-indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400 transition-all' disabled={isGenerating}/>
                    <button className='absolute right-2.5 bottom-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition-colors disabled:opacity-60' disabled={isGenerating || !input.trim() }>
